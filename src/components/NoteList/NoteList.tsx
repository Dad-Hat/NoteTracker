...
export function NoteList({ availableTags, notes, onUpdateTag, onDeleteTag }: NoteListProps) {
    ...
    return (
        <>
            <Row className="align-items-center mb-4">
                ...
                <Col xs="auto">
                    <Stack gap={2} direction="horizontal">
                        ...
                        <Button variant="outline-secondary" onClick={() => setEditTagsIsOpen(true)}>Edit Tags</Button>
                    </Stack>
                </Col>
            </Row>
            <Form>
            ...
            <EditTagsModal show={editTagsIsOpen} handleClose={() => setEditTagsIsOpen(false)} availableTags={availableTags} onUpdateTag={onUpdateTag} onDeleteTag={onDeleteTag} />
        </>
    )
}
...
function NoteCard({ id, title, tags }: SimplifiedNote) {
    return <Card as={Link} to={`/${id}`} className={`h-100 text-reset text-decoration-none ${styles.card}`}>
        <Card.Body>
            <Stack gap={2} className="align-items-center justifiy-content-center h-100">
